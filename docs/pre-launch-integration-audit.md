...
| ⑤ | 병행운영 오버셀 = 운영정책만(코드강제 없음) | 🟠 | ✅ 운영 체크리스트 문서화 |
...
### ⑤ 병행운영 오버셀 (🟠 — 운영) — ✅ 체크리스트 문서화
- 메이크샵↔Shopify 재고 싱크 코드 없음. `team-decisions.md`의 "상품 미겹침(A안)" 정책에만 의존.
- ✅ **`docs/parallel-oversell-checklist.md` 작성**: 상품 이전 시 **"① 메이크샵 내림 → ② Shopify active"** 순서 강제(겹침 창 제거) + 실재고 확정·겹침검증·이전 로그. ②가 Shopify 내부 오버셀은 막지만, 두 몰 교차 오버셀은 이 운영 규칙으로만 방어.
- ⚠️ 의존성: Shopify 상품 출고 재고 차감 방식(셀메이트 대체) 미정 → 결정 전까지 이전 소수 제한 + 수동 차감(ops-workflow-checklist 5).
