...
| [ ] | 품절 상품 처리 정책 결정 | 숨김 vs 품절 표시 — **담당자 문의중**. 재입고 알림 활용하려면 '표시' 권장 |
...
| [ ] | 색상 대비 WCAG AA | 상품/장바구니 **가격**을 `text-ink`로 개선 완료. `ink-muted #9A8F88` 토큰 자체는 AA 미달(보조 텍스트) — 토큰 변경은 브랜드 디자인 결정 필요 |
