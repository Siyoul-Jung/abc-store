...
| [x] | 현금영수증 발급 체계 | ✅ 계좌이체 현금영수증 **사용** 확정(상점관리자). ✅ **체크아웃 계좌이체 선택 시 현금영수증 발급 안내 문구** 반영(`CheckoutForm.tsx`, ko 전용, 2026-09-08) |
| [x] | 에스크로(구매안전서비스) 표시 | ✅ 계좌이체 **에스크로 사용** 확정. ✅ 푸터 "구매안전서비스 가입사실 확인" 링크 + 확인증 페이지 `/[lang]/escrow`(A08-260827-0001). ✅ **체크아웃 계좌이체 선택 시 구매안전서비스 안내 + 확인 링크**(`CheckoutForm.tsx`, ko 전용, 2026-09-08) |
...
| [x] | 재입고 알림 신청 | 품절 variant 선택 시 이메일 폼 (`RestockNotify.tsx` + `restock.ts`). 운영 Supabase DDL 실행 완료(2026-06-23) |
